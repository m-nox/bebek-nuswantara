...
try {
  ...
  console.log("Could not initialize DANA Client. Waiting for valid keys.", e.message);
}
...
app.post('/api/orders', async (req, res) => {
  ...
  if (paymentApi && process.env.DANA_PRIVATE_KEY && process.env.DANA_PRIVATE_KEY.length > 50) {
      ...
      } catch (error) { console.error('DANA API Error:', error); }
  }

  return res.json({ success: true, orderId: newOrder.id, paymentUrl: checkoutUrl, message: 'DANA checkout generated via Vercel.' });
});
